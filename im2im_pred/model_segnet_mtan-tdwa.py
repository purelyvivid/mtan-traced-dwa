...
parser.add_argument('--gpus', default='0', type=str, help='gpus')#added 20200609
parser.add_argument('--second_order_ratio', default=0.1, type=float, help='between 0 to 1')#added 20200609
...
device = torch.device("cuda:"+opt.gpus if torch.cuda.is_available() else "cpu")
...
for epoch in range(total_epoch):
    ...
    if opt.weight == 'dwa' or opt.weight == 'tdwa':
    ...
    for k in range(train_batch):
        ...
        if opt.weight == 'equal' or opt.weight == 'dwa' or (opt.weight == 'tdwa' and index <1):
            loss = sum([lambda_weight[i, index] * train_loss[i] for i in range(3)])
        elif opt.weight == 'tdwa':#added 20200609 
            weight_this = lambda_weight[:, index] #(3,)
            weight_prev = lambda_weight[:, index-1] #(3,) 
            weight = (1.0-opt.second_order_ratio)*weight_this + opt.second_order_ratio*weight_prev*weight_this #(3,)
            loss = sum([weight[i] * train_loss[i] for i in range(3)])
        else:
